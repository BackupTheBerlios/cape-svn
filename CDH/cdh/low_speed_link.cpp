/* File:    low_speed_link.cpp
 * Author:  matt
 * Purpose: Implements the interface to the low speed modem.
 */
...
/* Get the path to the low speed tty's device. */
...
int LowSpeedLink::Open()
{
    ...
    fd = open(lowDevice, O_RDWR | O_NOCTTY | O_NDELAY);
    if(!fd)
    {
